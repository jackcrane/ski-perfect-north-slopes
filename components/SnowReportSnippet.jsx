    ...
    <>
      <View style={styles.snowbriefItems}>
        <View style={styles.snowbriefItem}>
          ...
          {snowbriefReady ? (
            <>
              <Text style={{fontSize: 30}}>{snowbrief?.slope_access.open_trails_count}/{snowbrief?.slope_access.total_trails_count}</Text>
            </>
          ) : (
        ...
        <View style={{...styles.snowbriefItem, marginHorizontal: 10}}>
          ...
          {snowbriefReady ? (
            <>
              <Text style={{fontSize: 30}}>{snowbrief?.slope_access.open_lifts_count}/{snowbrief?.slope_access.total_lifts_count}</Text>
            </>
          ) : (
            <ActivityIndicator size="large" />
          )}
        ...
        <View style={styles.snowbriefItem}>
          <Text adjustsFontSizeToFit={true} style={styles.snowbriefItemTitle}>Tubing</Text>
          {snowbriefReady ? (
            <>
              <Text style={{fontSize: 30}}>{snowbrief?.tubing_park.lanes_open}</Text>
              <Text>Lanes</Text>
            </>
          ) : (
            <ActivityIndicator size="large" />
