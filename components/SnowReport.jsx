  ...
  return (
    <ScrollView
      ...
      refreshControl={
        <RefreshControl
          refreshing={false}
          onRefresh={() => setRequestNewSnowReport(requestNewSnowReport + 1)}
        />
      }
      refreshing
      style={styles.TBShown_container}
    >
      {snowReport.comments ? (
        <>
          ...
          <View style={lstyles.group}>
            ...
            <View style={lstyles.row}>
              <Text style={lstyles.content}>
                New snow in the last 24 hours:{" "}
              </Text>
              <Text style={{ ...lstyles.content, ...lstyles.value }}>
                {snowReport.current_conditions.new_snow_24_hrs}"
              </Text>
            </View>
            <View style={lstyles.row}>
              <Text style={lstyles.content}>New snow in the last 7 days: </Text>
              <Text style={{ ...lstyles.content, ...lstyles.value }}>
                {snowReport.current_conditions.new_snow_7_days}"
              </Text>
            </View>
            <View style={lstyles.row}>
              <Text style={lstyles.content}>Snow Base: </Text>
              <Text style={{ ...lstyles.content, ...lstyles.value }}>
                {snowReport.current_conditions.snow_base}
              </Text>
            </View>
          </View>
          <View style={lstyles.group}>
            ...
            <View style={lstyles.row}>
              ...
              <Text style={{ ...lstyles.content, ...lstyles.value }}>
                {Object.values(snowReport.chair_lifts).filter((x) => x).length}
              </Text>
              <Text style={lstyles.content}> of </Text>
              <Text style={{ ...lstyles.content, ...lstyles.value }}>
                {Object.values(snowReport.chair_lifts).length}
              </Text>
            </View>
            <StatusComponent
              title="Green Chairlift"
              value={snowReport.chair_lifts.green_chair}
            />
            <StatusComponent
              title="Red Chairlift"
              value={snowReport.chair_lifts.red_chair}
            />
            <StatusComponent
              title="Blue Chairlift"
              value={snowReport.chair_lifts.blue_chair}
            />
            <StatusComponent
              title="Orange Chairlift"
              value={snowReport.chair_lifts.orange_chair}
            />
            <StatusComponent
              title="White Chairlift"
              value={snowReport.chair_lifts.white_chair}
            />
            <Text style={lstyles.content}>
              Open magic carpets and rope tows count:{" "}
              <Text style={{ ...lstyles.content, ...lstyles.value }}>
                {snowReport.slope_access.open_lifts_count}
              </Text>
              <Text style={lstyles.content}> of </Text>
              <Text style={{ ...lstyles.content, ...lstyles.value }}>
                {snowReport.slope_access.total_lifts_count}
              </Text>
            </Text>
          </View>
          <View style={lstyles.group}>
            <Text style={lstyles.title}>Slopes</Text>
            {Object.entries(snowReport.trail_status).map(
              ([trail, content], index) => (
                <StatusComponent
                  key={index}
                  prefix={
